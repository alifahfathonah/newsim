  ...
  public function simpanJadwalAsprakCSV()
  {
    ...
    if (strtolower(end($ekstensi_file)) === 'csv' && $_FILES['file_csv']['size'] > 0) {
      ...
      while (($row = fgetcsv($handle, 2048))) {
        ...
        $id_jadwal_lab  = $this->db->select('id_jadwal_lab')->from('jadwal_lab')->where('kelas', $kelas)->where('kode_dosen', $dosen)->where('hari_ke', $hari_ke)->where('id_mk', $id_mk)->get()->row()->id_jadwal_lab;
        $jadwal_asprak  = array(
          'nim_asprak'    => $row[0],
