...
<body>
  ...
  <table border="1" width="100%" class="table-isi">
    ...
    <tbody>
      <?php
      $ttd_dosen          = '<img src="' . base_url($dosen->ttd_dosen) . '" style="height: 60px" class="img-dosen">';
      foreach ($bap as $b) {
        $tanggal_indonesia  = tanggal_indonesia($b->tanggal);
        if ($b->screenshot == 'Praktikum Onsite') {
          $gambar_praktikum = $b->screenshot;
        } else {
  ...
  <table width="100%" style="text-align: right">
    <tr>
      <?php
      if ($dosen->tanggal_approve != 'xx') {
        echo '<td>Bandung, ' . tanggal_indonesia($dosen->tanggal_approve) . '</td>';
      } else {
        echo '<td>Bandung, XX XXX XXXX</td>';
      }
      ?>
    </tr>
    <tr>
    ...
    <tr>
      <td>
        <div>
          ...
          <?php
          if ($dosen->tanggal_approve != 'xx') {
            echo '<p class="p-dosen">' . $dosen->nama_dosen . '<br>' . tanggal_indonesia($dosen->tanggal_approve) . '</p>';
          } else {
            echo '<p class="p-dosen">' . $dosen->nama_dosen . '<br>XX XXX XXXX</p>';
          }
          ?>
        </div>
      </td>
